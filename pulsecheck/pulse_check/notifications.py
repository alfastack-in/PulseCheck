...
class _FallbackCache:
    ...
    def set_value(self, key: str, value: str, expires_in_sec: Optional[int] = None) -> None:
...
def extract_schedule(settings) -> Optional[Tuple[int, time]]:
...
def should_run_now(settings, now: Optional[datetime] = None, window_minutes: int = 30) -> bool:
...
def get_slack_token(settings) -> Optional[str]:
...
def already_executed(cache_key: str, now: Optional[datetime] = None) -> bool:
...
def mark_executed(cache_key: str, now: Optional[datetime] = None) -> None:
    if now is None:
        now = _now()
...
def get_week_bounds(now: Optional[datetime] = None, *, offset_weeks: int = 0) -> Tuple[date, date]:
